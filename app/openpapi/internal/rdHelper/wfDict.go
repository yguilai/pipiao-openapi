...
type WfDictHelper struct {
    redis  *redis.Redis
    client *github.Client
}

func NewWfDictHelper(r *redis.Redis) *WfDictHelper {
    return &WfDictHelper{
        redis:  r,
...
func (h *WfDictHelper) NeedFetch(ctx context.Context) (string, bool) {
    ctt, _, _, err := h.client.Repositories.GetContents(ctx, warframeDictOwner, warframeDictRepo, warframeDictPath, nil)
    if err != nil {
    ...
    // 获取上次拉取的
    lastSHA, err := h.redis.Get(warframeDictSyncSHARedisKey)
    if err != nil {
    ...
    err = h.redis.SetexCtx(ctx, warframeDictSyncSHARedisKey, *ctt.SHA, warframeDictSyncSHARedisExpire)
    if err != nil {
        // 这里就只告警一下吧
